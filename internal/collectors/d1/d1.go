...
func (c *groupsCollector) queryRows(ctx context.Context, request cfapi.GraphQLRequest) ([]map[string]any, error) {
	...
	if errors.As(err, &window) {
		if errors.Is(window.Reason, collector.ErrWindowIrreducible) {
			return nil, fmt.Errorf("D1 Groups query still saturates an irreducible five-minute window: %w", window.Cause)
		}
		return nil, fmt.Errorf("cannot split saturated D1 Groups window %s..%s: %w", window.From.Format(time.RFC3339), window.To.Format(time.RFC3339), window.Cause)
	}
	return rows, err
}
