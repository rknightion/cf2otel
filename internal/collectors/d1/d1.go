...
const (
	d1QueryLimit         = 10000
	d1BucketDuration     = 5 * time.Minute
	d1MinimumQueryWindow = 5 * time.Minute
)
...
var datasets = []datasetSpec{
	{
		collector: "d1.analytics",
		dataset:   "d1AnalyticsAdaptiveGroups",
		metrics: []metricSpec{
			{name: semconv.MetricD1ReadQueries, field: "sum.readQueries", kind: counterMetric},
			{name: semconv.MetricD1WriteQueries, field: "sum.writeQueries", kind: counterMetric},
		},
	},
	{
		collector: "d1.queries",
		dataset:   "d1QueriesAdaptiveGroups",
		metrics:   []metricSpec{{name: semconv.MetricD1Queries, field: "count", kind: counterMetric}},
	},
	{
		collector: "d1.storage",
		dataset:   "d1StorageAdaptiveGroups",
		metrics:   []metricSpec{{name: semconv.MetricD1StorageBytes, field: "max.databaseSizeBytes", kind: gaugeMetric}},
	},
}
...
func (c *groupsCollector) CollectWindow(ctx context.Context, from, to time.Time, out telemetry.Emitter) (time.Time, error) {
	...
	if c.cfg == nil || c.api == nil || c.cfg.Cloudflare.AccountID == "" {
		return from, errors.New("D1 Groups collector requires a configured Cloudflare account and API")
	}
	reader, ok := c.api.(settingsReader)
	if !ok {
		return from, errors.New("cloudflare client does not expose D1 Groups dataset settings")
	}
	settings, err := reader.DatasetSettings(ctx, cfapi.AccountScope, c.cfg.Cloudflare.AccountID, c.spec.dataset)
	if err != nil {
		return from, fmt.Errorf("read D1 Groups settings for %s: %w", c.spec.dataset, err)
	}
	if !settings.Enabled {
		return from, fmt.Errorf("D1 Groups dataset %s is disabled", c.spec.dataset)
	}
	if settings.MaxNumberOfFields <= 0 {
		return from, fmt.Errorf("D1 Groups dataset %s has an invalid field limit", c.spec.dataset)
	}
	if settings.MaxPageSize <= 0 {
		return from, fmt.Errorf("D1 Groups dataset %s has an invalid page-size limit", c.spec.dataset)
	}
	...
	limit := min(d1QueryLimit, settings.MaxPageSize)
...
func (c *groupsCollector) requiredFields(settings cfapi.DatasetSettings) ([]string, error) {
	...
	for _, field := range fields {
		if !hasAvailableField(settings.AvailableFields, field) {
			return nil, fmt.Errorf("D1 Groups dataset %s is missing required field %s", c.spec.dataset, field)
		}
	}
...
func (c *groupsCollector) queryRows(ctx context.Context, request cfapi.GraphQLRequest) ([]map[string]any, error) {
	...
	if request.To.Sub(request.From) <= d1MinimumQueryWindow {
		return nil, fmt.Errorf("D1 Groups query still saturates an irreducible five-minute window: %w", err)
	}
	mid, ok := bucketSplit(request.From, request.To)
	if !ok {
		return nil, fmt.Errorf("cannot split saturated D1 Groups window %s..%s: %w", request.From.Format(time.RFC3339), request.To.Format(time.RFC3339), err)
	}
	leftRequest := request
	leftRequest.To = mid
...
func (c *groupsCollector) aggregate(rows []map[string]any, from, to time.Time) ([]metricValue, error) {
	...
	for _, row := range rows {
		...
		if err != nil {
			return nil, fmt.Errorf("D1 Groups dataset %s returned an invalid five-minute bucket", c.spec.dataset)
		}
		if bucket.Before(from) || !bucket.Before(to) || bucket.Add(d1BucketDuration).After(to) {
			continue
		}
		...
		for _, metric := range c.spec.metrics {
			...
			if !ok {
				return nil, fmt.Errorf("D1 Groups dataset %s returned a row without valid %s", c.spec.dataset, metric.field)
			}
...
func floorBucket(value time.Time) time.Time {
	return value.UTC().Truncate(d1BucketDuration)
}
...
func ceilBucket(value time.Time) time.Time {
	...
	return floor.Add(d1BucketDuration)
}
