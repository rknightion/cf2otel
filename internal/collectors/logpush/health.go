...
func (c *healthMetrics) queryWindow(ctx context.Context, request cfapi.GraphQLRequest, from, to time.Time, limit int) ([]map[string]any, error) {
	request.Limit = limit
	rows, err := collector.Bisect(from, to, logpushBucket, logpushBucket, func(from, to time.Time) ([]map[string]any, bool, error) {
		leaf := request
	...
	if errors.As(err, &window) {
		if errors.Is(window.Reason, collector.ErrWindowIrreducible) {
			return nil, fmt.Errorf("logpush health query still saturates a five-minute bucket: %w", window.Cause)
		}
		return nil, fmt.Errorf("cannot split saturated Logpush health window %s..%s", window.From.Format(time.RFC3339), window.To.Format(time.RFC3339))
	}
	return rows, err
}
...
func logpushIsSaturation(err error) bool {
	...
	return ok && sat.Dataset == logpushDataset
}
