...
type fakeAPI struct {
	...
	missingResponse, missingRequest bool
}
...
func TestMissingRequestBodyKeepsResponse(t *testing.T) {
	api := &fakeAPI{
		list:   `{"result":[{"id":"A","created_at":"2026-09-23T10:00:00Z","path":"chat/completions","duration":1}]}`,
		detail: `{}`, response: `{"choices":[{"message":{"role":"assistant","content":"fixture"}}]}`,
		missingRequest: true,
	}
	cfg := &config.Config{Cloudflare: config.CloudflareConfig{AccountID: "example"}, AIGateway: config.AIGatewayConfig{Gateways: []string{"gateway"}, CaptureBodies: true, MaxBodyBytes: 1024}}
	out := &fakeEmitter{}
	...
	if len(out.spans) != 1 || attr(out.spans[0].Attrs, semconv.AttrAIGatewayRequestBodyUnavailable) != "true" || len(out.spans[0].Logs) != 1 || attr(out.spans[0].Logs[0].Attrs, semconv.AttrAIGatewayContentSide) != "response" {
		t.Fatal("missing request marker or response content absent")
	}
}
