...
## Acceptance Criteria
<!-- AC:BEGIN -->
- [x] #1 Flags: -config, -version, -healthcheck, -validate, -print-effective-config, -once, -dry-run, -since/-before window override, -datasets selection, -reset-state, and an explore mode that prints a dataset and names a missing permission on 403
- [x] #2 Poll interval clamped to a floor; one failing collector never stops the others
- [x] #3 Config-file permission advisory (0600, owned by the runtime UID) as in tailscale2otel
<!-- AC:END -->
