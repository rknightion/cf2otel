...
## Acceptance Criteria
<!-- AC:BEGIN -->
- [x] #1 internal/config loads defaults, YAML and CF2OTEL_ env (double-underscore nesting), rejects secrets found in YAML, validates all errors at once, and redacts secrets in every dump
- [x] #2 internal/semconv declares cloudflare.*, gen_ai.* and cf2otel.* names; nothing outside it declares a signal or attribute name
- [x] #3 internal/telemetry exposes the Emitter (gauge, counter, histogram, log event, span with explicit start/end timestamps and links) and is the only OTLP importer, with Grafana Cloud basic auth over OTLP http and grpc
- [x] #4 internal/collector provides SnapshotCollector and WindowCollector registration, a staggered scheduler with panic recovery, and an atomic same-directory file checkpoint store; a test proves a window is neither skipped nor double-counted across a restart
- [x] #5 One stub Register file per wave-1 domain (access, httpreq, aigateway, inventory, selfobs) exists and is called from cmd/cf2otel/collectors_import.go; just check passes
<!-- AC:END -->
